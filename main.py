...
class EasyApplyLinkedin:
    ...
    def job_search(self):
        ...
        search_keywords = self.driver.find_element_by_css_selector(
            ...
            ".jobs-search-box__text-input[aria-label='Job title, skill, or company']")
    ...
    def find_offers(self):
        ...
        for result in results:
            ...
            hover.perform()
            # companies = result.find_elements_by_class_name('job-card-search__company-name.artdeco-entity-lockup__subtitle.ember-view')
            titles = result.find_elements_by_class_name(
                'job-card-search__title.artdeco-entity-lockup__title.ember-view')
            companies2 = result.find_elements_by_class_name('job-card-search__company-name.artdeco-entity-lockup__subtitle.ember-view')
            
            
            for title in titles:
                for company in companies2:
                    self.submit_apply(title, company)
        ...
        if total_results_int > 24:
            ...
            for page_number in range(25, total_jobs+25, 25):
                ...
                for result_ext in results_ext:
                    ...
                    hover_ext.perform()
                    cname = result_ext.find_elements_by_class_name('job-card-search__company-name.artdeco-entity-lockup__subtitle.ember-view')
                    titles_ext = result_ext.find_elements_by_class_name(
                        'job-card-search__title.artdeco-entity-lockup__title.ember-view')
                    companies2_ext = result_ext.find_elements_by_class_name('job-card-search__company-name. artdeco-entity-lockup__subtitle.ember-view')
                    for title_ext in titles_ext:
                        for company_ext in companies2_ext:
                            self.submit_apply(title_ext, company_ext)
        else:
            self.close_session()

    def submit_apply(self, job_add, companyname):
        """This function submits the application for the job add found"""

        print('You are applying to the position of: ', job_add.text, companyname.text)
        
        # lcname = self.driver.find_element_by_class_name('job-card-search__company-name.artdeco-entity-lockup__subtitle.ember-view')
        
        # cname = self.driver.find_element_by_class_name('job-details-top-card__company-url.ember-view')
        # print(cname)
        # print(cname.text)
        time.sleep(1)
        job_add.click()
        ...
        try:
            ...
            # cname = self.driver.find_elements_by_xpath(
            #     "//a[@class='jobs-details-top-card__company-url ember-view']/a")
            # print(cname)
            # print(cname.get_attribute("text"))
            
           

            connection = psycopg2.connect(user="yourusernamehere",
                                      password="null",
                                      host="hosthere,
                                      port="porthere",
            ...
            cursor.execute(
            "INSERT INTO appliedjobs (jobdata, companyname) VALUES(%s, %s);", (job_add.text, companyname.text))
            connection.commit()
            cursor.close()
        # count = cursor.rowcount
            connection.close()
            close_window = self.driver.find_element_by_class_name(
